...
export default function AdminCategories() {
  ...
  useEffect(() => { getCategories().then(r => setItems(r.data)) }, [])
  ...
  return (
    <div className="p-6 max-w-xl">
      <h1 className="text-xl font-bold text-white mb-6">Категории</h1>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl divide-y divide-gray-800">
        {items.length === 0 ? (
          <p className="p-5 text-sm text-gray-500">Нет категорий</p>
        ) : items.map(cat => (
          <div key={cat.category_id} className="flex items-center justify-between px-5 py-3.5 hover:bg-gray-800/30 transition-colors">
            <span className="text-sm text-gray-200">{cat.category_name}</span>
            <button onClick={() => handleDelete(cat.category_id)} className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors">
              <Trash2 size={14} />
            </button>
          </div>
        ))}
